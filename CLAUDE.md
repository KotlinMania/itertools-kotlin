# Claude Code Project Instructions — itertools-kotlin

## Project Overview

This is **itertools-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`itertools`](https://crates.io/crates/itertools).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/itertools/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/itertools>
