...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`itertools`](https://crates.io/crates/itertools).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `itertools` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Extra iterator adapters
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:itertools-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `itertools` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
